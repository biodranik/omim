...
void FeatureType::ApplyPatch(osm::EditableFeature const & ef)
{
  if (feature::GEOM_POINT == GetFeatureType())
    m_center = ef.GetMercator();

  m_params.name = ef.GetName();
  string const & house = ef.GetHouseNumber();
  if (house.empty())
  ...
  m_metadata = ef.GetMetadata();
  m_bMetadataParsed = true;

  uint32_t typesCount = 0;
  for (uint32_t const type : ef.GetTypes())
