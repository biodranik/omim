...
UNIT_TEST(SunriseSunsetAlgorithm_Moscow_April)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 4, 12, lat, lon);
...
UNIT_TEST(SunriseSunsetAlgorithm_Moscow_July)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 7, 13, lat, lon);
...
UNIT_TEST(SunriseSunsetAlgorithm_Moscow_September)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 9, 17, lat, lon);
...
UNIT_TEST(SunriseSunsetAlgorithm_Moscow_December)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 12, 25, lat, lon);
...
UNIT_TEST(SunriseSunsetAlgorithm_Moscow_NewYear_1)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2016, 1, 1, lat, lon);
...
UNIT_TEST(SunriseSunsetAlgorithm_GetDayTime_Moscow_December)
{
  ...
  pair<DayTimeType, time_t> dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 12, 23, 17, 0, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
  TEST(TimesEqual(dayTime.second, base::TimeGM(2015, 12, 24, 6, 0, 0)), ());

  // between sunrise and sunset
  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 12, 24, 10, 53, 0), lat, lon);
  ...
  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 12, 24, 16, 30, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
...
UNIT_TEST(SunriseSunsetAlgorithm_Honolulu_February)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 2, 12, lat, lon);
  ...
  auto dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 2, 12, 15, 15, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.first, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 2, 12, 17, 10, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.second, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 2, 13, 2, 30, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.second, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 2, 13, 5, 0, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
}
...
UNIT_TEST(SunriseSunsetAlgorithm_Honolulu_June)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 6, 22, lat, lon);
  ...
  sunriseSunset = SunriseSunsetForUtc(base::TimeGM(2015, 6, 22, 15, 0, 0), lat, lon);
  TEST(TimesEqual(sunriseSunset.first, base::TimeGM(2015, 6, 22, 15, 51, 0)), ());
  TEST(TimesEqual(sunriseSunset.second, base::TimeGM(2015, 6, 23, 5, 17, 0)), ());

  sunriseSunset = SunriseSunsetForUtc(base::TimeGM(2015, 6, 22, 19, 0, 0), lat, lon);
  TEST(TimesEqual(sunriseSunset.first, base::TimeGM(2015, 6, 22, 15, 51, 0)), ());
  TEST(TimesEqual(sunriseSunset.second, base::TimeGM(2015, 6, 23, 5, 17, 0)), ());

  sunriseSunset = SunriseSunsetForUtc(base::TimeGM(2015, 6, 23, 1, 0, 0), lat, lon);
  TEST(TimesEqual(sunriseSunset.first, base::TimeGM(2015, 6, 22, 15, 51, 0)), ());
  TEST(TimesEqual(sunriseSunset.second, base::TimeGM(2015, 6, 23, 5, 17, 0)), ());

  sunriseSunset = SunriseSunsetForUtc(base::TimeGM(2015, 6, 23, 5, 10, 0), lat, lon);
  TEST(TimesEqual(sunriseSunset.first, base::TimeGM(2015, 6, 22, 15, 51, 0)), ());
  TEST(TimesEqual(sunriseSunset.second, base::TimeGM(2015, 6, 23, 5, 17, 0)), ());
}
...
UNIT_TEST(SunriseSunsetAlgorithm_Honolulu_December)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 12, 23, lat, lon);
...
UNIT_TEST(SunriseSunsetAlgorithm_GetDayTime_Melbourne_August)
{
  ...
  // before sunrise but after prev sunset
  pair<DayTimeType, time_t> dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 8, 11, 15, 0, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
  TEST(TimesEqual(dayTime.second, base::TimeGM(2015, 8, 11, 21, 10, 0)), ());

  // between sunrise and sunset
  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 8, 11, 21, 20, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST(TimesEqual(dayTime.second, base::TimeGM(2015, 8, 12, 7, 42, 0)), ());

  // after sunset but before next sunrise
  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 8, 12, 10, 0, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
...
UNIT_TEST(SunriseSunsetAlgorithm_Wellington_October)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 10, 20, lat, lon);
...
UNIT_TEST(SunriseSunsetAlgorithm_BuenosAires_March)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 3, 8, lat, lon);
  ...
  auto dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 3, 8, 9, 0, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.first, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 3, 8, 21, 0, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.second, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 3, 8, 22, 55, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
}
...
UNIT_TEST(SunriseSunsetAlgorithm_Seattle_May)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 5, 9, lat, lon);
...
UNIT_TEST(SunriseSunsetAlgorithm_CapeTown_November)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 11, 11, lat, lon);
...
UNIT_TEST(SunriseSunsetAlgorithm_CapeTown_March)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 3, 1, lat, lon);
...
UNIT_TEST(SunriseSunsetAlgorithm_Tiksi_July)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 7, 1, lat, lon);
  ...
  auto dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 7, 1, 0, 0, 0), lat, lon);
...
UNIT_TEST(SunriseSunsetAlgorithm_Tiksi_December)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 12, 1, lat, lon);
  ...
  pair<DayTimeType, time_t> dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 12, 1, 0, 0, 0), lat, lon);
...
UNIT_TEST(SunriseSunsetAlgorithm_Norilsk_NewYear)
{
  ...
  auto dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2016, 1, 1, 0, 0, 0), lat, lon);
  TEST_EQUAL(DayTimeType::PolarNight, dayTime.first, ());
}
...
UNIT_TEST(SunriseSunsetAlgorithm_Norilsk_August)
{
  ...
  pair<DayTimeType, time_t> dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 6, 22, 0, 0, 0), lat, lon);
...
UNIT_TEST(SunriseSunsetAlgorithm_Tokio_September)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 9, 12, lat, lon);
...
UNIT_TEST(SunriseSunsetAlgorithm_Kabul_March)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2015, 3, 20, lat, lon);
  ...
  auto dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 3, 20, 1, 15, 0), lat, lon);
  TEST_EQUAL(DayTimeType::NightTime, dayTime.first, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 3, 20, 1, 45, 0), lat, lon);
  TEST_EQUAL(DayTimeType::DayTime, dayTime.first, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 3, 20, 13, 15, 0), lat, lon);
  TEST_EQUAL(DayTimeType::DayTime, dayTime.first, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 3, 20, 13, 45, 0), lat, lon);
  TEST_EQUAL(DayTimeType::NightTime, dayTime.first, ());
}
...
UNIT_TEST(SunriseSunsetAlgorithm_Avarua_January)
{
  ...
  auto dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2016, 1, 1, 15, 15, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.first, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2016, 1, 1, 16, 10, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.second, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2016, 1, 2, 3, 15, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.second, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2016, 1, 2, 5, 35, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
}
...
UNIT_TEST(SunriseSunsetAlgorithm_Lorino_February)
{
  ...
  pair<time_t, time_t> sunriseSunset = SunriseSunsetForUtcDay(2016, 2, 3, lat, lon);
  ...
  auto dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2016, 2, 2, 19, 30, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.first, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2016, 2, 2, 20, 30, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.second, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2016, 2, 3, 3, 0, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.second, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2016, 2, 3, 10, 0, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
}
...
UNIT_TEST(SunriseSunsetAlgorithm_Anadyr_December)
{
  ...
  auto dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 12, 24, 19, 0, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.first, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 12, 24, 22, 30, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.second, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 12, 25, 1, 55, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.second, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 12, 25, 2, 30, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
}
...
UNIT_TEST(SunriseSunsetAlgorithm_Nikolski_December)
{
  ...
  auto dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 12, 25, 19, 0, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.first, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 12, 25, 19, 35, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.second, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 12, 26, 3, 0, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.second, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 12, 26, 3, 10, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
}
...
UNIT_TEST(SunriseSunsetAlgorithm_Kiribati_July)
{
  ...
  auto dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 7, 1, 16, 10, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.first, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 7, 1, 16, 35, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.second, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 7, 2, 4, 0, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.second, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 7, 2, 4, 50, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
}
...
UNIT_TEST(SunriseSunsetAlgorithm_Kiribati_July_2)
{
  ...
  sunriseSunset = SunriseSunsetForUtc(base::TimeGM(2015, 7, 2, 3, 30, 0), lat, lon);
  TEST(TimesEqual(sunriseSunset.first, base::TimeGM(2015, 7, 1, 16, 28, 0)), ());
  TEST(TimesEqual(sunriseSunset.second, base::TimeGM(2015, 7, 2, 4, 41, 0)), ());

  // afer sunset but before next sunrise
  sunriseSunset = SunriseSunsetForUtc(base::TimeGM(2015, 7, 2, 7, 0, 0), lat, lon);
  ...
  // afer sunset but before next sunrise
  sunriseSunset = SunriseSunsetForUtc(base::TimeGM(2015, 7, 2, 16, 10, 0), lat, lon);
  TEST(TimesEqual(sunriseSunset.first, base::TimeGM(2015, 7, 2, 16, 28, 0)), ());
  TEST(TimesEqual(sunriseSunset.second, base::TimeGM(2015, 7, 3, 4, 42, 0)), ());
}
...
UNIT_TEST(SunriseSunsetAlgorithm_London_July)
{
  ...
  auto dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 7, 1, 2, 50, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.first, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 7, 1, 16, 20, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.second, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 7, 1, 20, 10, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::DayTime, ());
  TEST_EQUAL(dayTime.second, sunriseSunset.second, ());

  dayTime = GetNextUtcSunriseOrSunset(base::TimeGM(2015, 7, 1, 21, 15, 0), lat, lon);
  TEST_EQUAL(dayTime.first, DayTimeType::NightTime, ());
}
